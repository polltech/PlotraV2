...
from sqlalchemy import select, func, false as sqlalchemy_false
...
from app.models.payments import IncentiveRule, PaymentEscrow, DigitalSignature, PayoutStatus
from app.models.compliance import DigitalProductPassport
...
async def create_practice_log(
    ...
    practice_log = PracticeLog(
        parcel_id=parcel_id,
        event_type=practice_type,
        event_date=practice_date,
...
async def get_practice_logs(
    ...
    """Get practice logs for user's parcels or all logs for admins."""
    
    # Platform admins and EUDR reviewers can see all logs
    is_admin = current_user.role in [UserRole.PLOTRA_ADMIN, UserRole.EUDR_REVIEWER]
    
    if is_admin:
        # Admins see all practice logs
        query = select(PracticeLog).join(LandParcel).join(Farm)
    elif current_user.role == UserRole.COOPERATIVE_OFFICER and current_user.cooperative_id:
        # Coop officers see logs from their cooperative
        query = select(PracticeLog).join(LandParcel).join(Farm).where(
            Farm.cooperative_id == current_user.cooperative_id
        )
    else:
        # Farmers see only their own logs
        query = select(PracticeLog).join(LandParcel).join(Farm).where(Farm.owner_id == current_user.id)
    
    if parcel_id:
        query = query.where(PracticeLog.parcel_id == parcel_id)
    
    result = await db.execute(query.order_by(PracticeLog.event_date.desc()))
    ...
    return [
        {
            ...
            "practice_type": log.event_type,
            "practice_date": log.event_date.isoformat(),
...
async def get_dashboard_summary(
    ...
    # Base query for parcels - filter by cooperative if specified
    parcels_query = select(LandParcel).join(Farm)
    
    # Platform admins see all data, others are filtered
    is_admin = current_user.role in [UserRole.PLOTRA_ADMIN, UserRole.EUDR_REVIEWER]
    
    if cooperative_id:
        parcels_query = parcels_query.where(Farm.cooperative_id == str(cooperative_id))
    elif current_user.role == UserRole.FARMER:
        parcels_query = parcels_query.where(Farm.owner_id == current_user.id)
    elif current_user.role == UserRole.COOPERATIVE_OFFICER:
        # Look up cooperative via membership table
        from app.models.user import CooperativeMember
        mem_result = await db.execute(
            select(CooperativeMember.cooperative_id).where(
                CooperativeMember.user_id == current_user.id, CooperativeMember.is_active == True
            ).limit(1)
        )
        coop_id = mem_result.scalar_one_or_none()
        if coop_id:
            parcels_query = parcels_query.where(Farm.cooperative_id == coop_id)
    # Platform admins see all - don't add any filter
    ...
    # 2. Carbon Stored - sum of biomass from parcels and biomass ledgers
    # First get from parcel biomass
    carbon_from_parcels = sum([p.biomass_tons or 0 for p in parcels])
    
    # Also check biomass ledgers for more accurate data
    ...
    if parcel_ids:
        ...
        carbon_from_ledgers = ledger_result.scalar() or 0.0
        total_carbon = max(carbon_from_parcels, carbon_from_ledgers)
    else:
        total_carbon = carbon_from_parcels
    ...
    if ndvi_values or canopy_values:
        ...
        # Normalize to 0-10 scale (NDVI is typically 0-1, canopy is 0-1)
        soil_health_score = round((avg_ndvi * 5 + avg_canopy * 5), 1)
    else:
    ...
    if cooperative_id:
        # Get farmers in this cooperative via membership table
        from app.models.user import CooperativeMember
        ...
        farmer_ids = farmers_result.scalars().all()
        if farmer_ids:
            incentive_query = incentive_query.where(IncentiveClaim.farmer_id.in_(farmer_ids))
        else:
            incentive_query = incentive_query.where(sqlalchemy_false())
    elif current_user.role == UserRole.FARMER:
    ...
    logs_query = select(PracticeLog).join(LandParcel).join(Farm)
    
    if cooperative_id:
        logs_query = logs_query.where(Farm.cooperative_id == cooperative_id)
    elif current_user.role == UserRole.FARMER:
        logs_query = logs_query.where(Farm.owner_id == current_user.id)
    elif current_user.role == UserRole.COOPERATIVE_OFFICER:
        from app.models.user import CooperativeMember
        ...
        coop_id2 = mem_result2.scalar_one_or_none()
        if coop_id2:
            logs_query = logs_query.where(Farm.cooperative_id == coop_id2)
    
    logs_result = await db.execute(logs_query.order_by(PracticeLog.event_date.desc()).limit(10))
    practice_logs = logs_result.scalars().all()
    
    recent_logs = []
    for log in practice_logs:
        parcel = await db.get(LandParcel, log.parcel_id)
        parcel_name = parcel.parcel_name or parcel.parcel_number if parcel else "Unknown"
        recent_logs.append({
            "id": log.id,
            "date": log.event_date.isoformat() if log.event_date else None,
            "parcel": parcel_name,
